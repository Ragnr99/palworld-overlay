...
game's own `DT_PalQuestData`. Each quest is keyed by its internal id
(`Main_UnlockFastTravel`, `Sub_Farmer04`), so the snapshot survives a site
redesign and can be diffed against the `.pak`.
...
- The **58 main missions** are one chain about 30 quests long. paldb records two
  tutorial entry points that run separately for a dozen steps and then converge.
  The browser reads from the one a new save opens with and lists the other behind
  it; `questdata.MAIN_ENTRY` picks which leads.
- **57 of the 59 side missions carry no links.** What unlocks them lives in
  Blueprint graphs the datamine never reached. They're shelved by questgiver,
  which their ids carry, and a **Nearby** list built from objective coordinates
  ties them back together - two quests within 150 units of each other are a real
  "while you're here".

Two cosmetic gaps:

- Briefings carry 68 name placeholders, shown as `{KingWhale}`. They're the
  game's own substitution tokens and paldb ships them unmade. The ids are rarely
  display names - `KingWhale` is the Pal you know as Panthalus - so they appear
  exactly as the game writes them. Resolving them means pulling
  `DT_ItemNameText` / `DT_PalNameText` out of the `.pak`.
